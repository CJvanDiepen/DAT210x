...
X = pd.read_csv(r"C:\Users\diepencjv\repos\DAT210x\Module6\Datasets\agaricus-lepiota.data", names=names)
...
with open("wheat.dot", 'w') as f:
    f = export_graphviz(dt, out_file=f, class_names=['poisonous', 'edible'])

#%%
importances = {x: 0 for x in X.columns}

for idx, weight in enumerate(dt.feature_importances_):
    importances[X.columns[idx]] = weight
